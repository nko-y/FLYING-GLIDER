...
namespace Snow {
	Snow::Snow()
	{
		...
		mUpdateShader = new SnowShader("Update.vs", "Update.fs", 
						"Update.gs", varyings, 5);
		//设置TransformFeedback缓存能够记录的顶点的数据类型

		mRenderShader = new SnowShader("Render.vs", 
							"Render.fs");
		//设置随机纹理
		InitRandomTexture(512);
		mSparkTexture.loadTexture("resources/textures/snowstorm.bmp");
		mRenderShader->use();
